...
parfor model_id =1:length(filelist)
    ...
    mesh_stress=load(fullfile(filelist(model_id).folder, filelist(model_id).name));
    shape = struct('X',mesh_stress.X(:,1),'Y',mesh_stress.X(:,2),'Z',mesh_stress.X(:,3),'TRIV',mesh_stress.F,'J2m',mesh_stress.J2m);
    mesh_stress = [];
